...
class ProductSquare extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final priceLabel = product.getOrNull<PriceLabelComponent>()?.value ?? '---';
    return Material(
      color: color,
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            priceLabel,
            style: TextStyle(
                color: isDark(color) ? Colors.grey : Colors.black54,
                fontSize: 24
            ),
          ),
          Row(
            ...
            children: <Widget>[
              IconButton(
                icon: Icon(Icons.add),
                onPressed: ()=> product + AddToShoppingCartComponent(),
              ),
              EntityObservingWidget(
                provider: (em)=>product,
                builder: (e, context) => IconButton(
                  icon: Icon(Icons.remove),
                  onPressed: product.has(CountComponent) ? ()=> product + RemoveFromShoppingCartComponent() : null,
                )
              ),
            ],
          )
